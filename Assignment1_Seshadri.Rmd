...
```{r Sample Frame, echo = FALSE, message=FALSE, warning=FALSE, tidy=TRUE,width = 50}
library(magrittr)
waterfall <- ames %>%
...
```{r Report data quality, echo=FALSE}
...
```{r ControlChart & Hist, echo=FALSE, fig.cap="Control Chart and Histogram", fig.show='hold', message=FALSE, warning=FALSE, out.width=c('250px','200px')}
...
```{r Boxplot Neighborhood, echo=FALSE, fig.cap="Boxplot of Sales by Neighborhood", fig.height=3, message=FALSE, warning=FALSE}
...
```{r Neighborhood barchart, fig.cap="Frequency of Sales by Neighborhood", fig.height=3}
...
```{r Sales by condition, echo=FALSE, fig.cap="Sales by Sale condition", fig.height=3, message=FALSE, warning=FALSE}
...
```{r Kitchen Quality, echo=FALSE, fig.cap="Sale price by kitchen quality assesment", fig.height=3, message=FALSE, warning=FALSE}
...
\pagebreak  
 ...
 From Figure 6, we can infer that there are more style of houses vailable for sale in lots in the inside of a community than Corners, Cul-de-sac or more frontage lots. Also, Cul-de-sac and 3 frontal properties are valued more compared to the test of the lots (see Figure 7). 
 \newline
 
 
```{r HouseStyle, echo=FALSE, fig.cap="Sale price by lot configuration and style", fig.height=4, message=FALSE, warning=FALSE}
...
```{r Lot config, echo=FALSE, fig.cap="Probability density of Sale price by lot configuration", fig.height=2, message=FALSE, warning=FALSE}
...
In the previous section we hypothesized that the sale price of homes are likely related to build year or the remodeled year. We'll use the year of remodel year for exploration. We find in figure 8 that newer homes are more expensive than the older homes. There is no strong relationship between the remodel year and the sale prive with a correlation of only `r as.numeric(format(cor(SampleFrame$YearRemodel, SampleFrame$SalePrice),digit = 2))`. However, it will be interesting to explore in which neighborhoods there may be much stronger relationship between the two variables. We'll explore this in the next section.

```{r Year Remodel, echo=FALSE, fig.cap="Sale price Vs Year Remodeled", fig.height=3, message=FALSE, warning=FALSE}
p <- ggplot(data = SampleFrame) + theme_bw()
p + geom_point(mapping = aes(x = YearRemodel, y = SalePrice)) + geom_smooth(mapping = aes(x = YearRemodel, y = SalePrice),method = 'loess', se = T) + ylab('Sale Price')
```  
...
```{r Rooms, echo=FALSE, fig.cap="Sale price Vs Number of rooms above ground", fig.height=3, message=FALSE, warning=FALSE}
...
```{r Basement area, echo=FALSE, fig.cap="Sale price Vs Total basement area", fig.height=3, message=FALSE, warning=FALSE}
...
It is assumed that bigger homes are of high value and are built for high occupancy. High occupancy homes are likely to have more bathrooms and living area (see Figure 12). The total number of bathrooms is a sum of the half baths and the full baths in the house. The total number of baths may be a week predictor of sale price by itself (Figure 11), but may be a good candidate for multivariate regression models among others.
  
    
    
```{r TotalBath, echo=FALSE, fig.cap="Sale price Vs Number  baths", fig.height=3, message=FALSE, warning=FALSE}
library(gridExtra)
SampleFrame <- SampleFrame %>% 
...
```{r Living Area, echo=FALSE, fig.cap="Sale price for high occupancy homes", fig.height=3, message=FALSE, warning=FALSE}
...
```{r LogSales vs LogLivArea, echo=FALSE, fig.cap="Log Sale price vs Log Living Area", fig.height=3, message=FALSE, warning=FALSE}
SampleFrame <- SampleFrame %>% 
  mutate(LogSalePrice = log10(SalePrice)) %>% 
...
```{r SaleCondition partial, echo=FALSE, fig.cap="Sale price vs Sale condition group", fig.height=4, message=FALSE, warning=FALSE}
