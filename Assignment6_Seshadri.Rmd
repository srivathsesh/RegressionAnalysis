...
We will split the scores into training and test sets for modelling. 70% of the data is chosen in random as the training set and the remaining of data is used as validation set. 
...
A linear regression model with the 8 principle component scores is fit. The results are summarized below. It is seen that VIF is 1. This is not surprising because the principle components are by design are orthogonal to each other. The eigenvectors are not correlated and orthogonal to each other. 
